		...
		categories: [
			{
				...
				exports: [
					...
					{ name: "Query", kind: "type", signature: "Query<C>", description: "Iterable query result with .each and .collect helpers." },
					{ name: "QueryOpts", kind: "type", signature: "{ without?: Component<any>[] }", description: "Optional excludes for query()." },
					{ name: "ComponentTuple", kind: "type", description: "Helper that maps a tuple of Component<T> to a tuple of T." },
					{ name: "WorldInternal", kind: "type", description: "Escape-hatch interface exposing internal stores for snapshot/restore." },
				],
			},
			{
				name: "Schedule",
			...
			},
			{
				...
				exports: [
					...
					{ name: "anim_c", kind: "constant", signature: "Component<AnimData>", description: "Per-entity animation state component." },
					{ name: "atlas_registry", kind: "constant", signature: "ResKey<AtlasRegistry>", description: "Resource key for the registered atlas sequences." },
					{ name: "anim_events", kind: "constant", signature: "ResKey<AnimEventBuffer>", description: "Resource key for the per-tick anim event buffer." },
					{ name: "Anim", kind: "type", description: "Animation controller interface." },
					{ name: "AnimData", kind: "type", description: "Per-entity animation state (sequence, frame, time, speed)." },
			...
			},
			{
				...
				exports: [
					{ name: "replay", kind: "namespace", signature: "{ record, record_engine, play, save, load, schema }", description: "Namespace bundling recorder, player, save/load helpers and the zod schema." },
	...
	},
	{
		...
		categories: [
			...
			},
			{
				...
				exports: [
					{ name: "assets", kind: "factory", signature: "(opts?: AssetsOpts) => Assets", description: "Create the asset loader. Provides image(), atlas(), registry()." },
					{ name: "Assets", kind: "type", description: "{ image, atlas, registry, get, dispose }." },
					{ name: "AssetsOpts", kind: "type", description: "{ fixed_dt, register_default? } options." },
					{ name: "AssetError", kind: "type", description: "Asset error union — fetch, parse, decode." },
				],
			},
			{
				name: "Browser source",
