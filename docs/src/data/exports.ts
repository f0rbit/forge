		...
		categories: [
			...
			},
			{
				...
				exports: [
					...
					{ name: "SpriteData", kind: "type", description: "Config-only: `{ texture, frame?, anchor?, tint?, visible?, z?, scale?, alpha? }`. The PIXI runtime node is owned by `sprite_sync_system` (private `WeakMap`); no `node` field on the public type." },
					...
					{ name: "SpriteData.alpha", kind: "type", signature: "number", description: "Optional 0..1 alpha applied to the underlying PIXI Sprite each frame. Default 1.0 (fully opaque). Pair with `Grid.lit_area` to drive gradient FOV — set per-tile sprite alpha from the lit-area intensity." },
		...
		categories: [
			{
				...
				exports: [
					...
					{ name: "grid.lit_area", kind: "function", signature: "(opts: LightOpts) => Map<number, number>", description: "Gradient FOV — same Bresenham LOS test as `line_of_sight`, but returns a `Map<key, intensity>` where intensity is a float in [0, 1]. Default falloff is linear `1 - distance/radius`. Pass a custom `falloff` for quadratic / smoothstep / torch flicker. Origin is always 1.0." },
					...
					{ name: "LightOpts", kind: "type", signature: "{ from: Cell; radius: number; is_blocking: (cell: Cell) => boolean; falloff?: (distance: number, max: number) => number }", description: "Options for `grid.lit_area`. `falloff` defaults to linear `(d, max) => 1 - d/max`. Custom falloffs are clamped to [0, 1]." },
