...
## Pairing with `move_tile`

When you call `move_tile` inside a periodic system, the gate handles cadence — the system body just performs one step:

```ts
const movement_system: System = (w) => {
  for (const [id, , d] of w.query_data([pos_c, dir_c], [player_c])) {
    if (d.dx === 0 && d.dy === 0) continue;
    move_tile(w, id, pos_c, g, d, { blocked_by });
  }
};

sch.add_periodic("update", movement_system, { every: ticks_per_step(8, fixed_dt) }, "movement");
```
