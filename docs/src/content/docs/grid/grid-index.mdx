...
const ai_system: System = (w, ctx) => {
  for (const [id, p] of w.query_data([pos_c], [enemy_c]).collect()) {
...
- `refresh()` is called eagerly on construction. After that, refresh on a cadence — typically every tick in `pre`, or only when entities move (use `add_periodic` with `every: N` if your game updates positions infrequently).
